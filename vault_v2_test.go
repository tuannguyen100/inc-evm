...
func (v2 *VaulV2TestSuite) TestVaultV2RequestWithdraw() {
	...
	_, err = v2.p.v.RequestWithdraw(auth2, IncPaymentAddr, tinfo.addr, deposit, signBytes, timestamp, txId, signBytesRegulator)
	...
	_, err = v2.p.v.RequestWithdraw(auth2, IncPaymentAddr, tinfo.addr, big.NewInt(0).Mul(redeposit, big.NewInt(int64(1e9))), signBytes, timestamp, txId, signBytesRegulator)
...
func (v2 *VaulV2TestSuite) TestVaultV2isSigDataUsed() {
	...
	_, err = v2.p.v.RequestWithdraw(auth2, IncPaymentAddr, tinfo.addr, testAmount, signBytes, timestamp, txId, signBytesRegulator)
