...
contract Vault {
    ...
    */
    ...
    enum Prefix {
        NOT_USE_EXECUTE_SIGNATURE,
        NOT_USE_REQUEST_WITHDRAW_SIGNATURE,
        NOT_USE_BSC_EXECUTE_SIGNATURE,
        NOT_USE_BSC_REQUEST_WITHDRAW_SIGNATURE,
    ...
    function withdraw(
        ...
        require(data.meta == 153 && data.shard == 1, errorToString(Errors.INVALID_DATA)); // Check instruction type
    ...
    function requestWithdraw(
        ...
        address verifier = verifySignData(abi.encode(newPreSignData(Prefix.PLG_REQUEST_WITHDRAW_SIGNATURE, token, timestamp, amount), incognitoAddress), signData);
    ...
    function execute(
        ...
        address verifier = verifySignData(abi.encode(newPreSignData(Prefix.PLG_EXECUTE_SIGNATURE, token, timestamp, amount), recipientToken, exchangeAddress, callData), signData);
    ...
    function callExtFunc(address recipientToken, uint ethAmount, bytes memory callData, address exchangeAddress) internal returns (uint) {
        ...
        require(success, errorToString(Errors.INTERNAL_TX_ERROR));
        require(balanceOf(recipientToken) <= 10 ** 27, errorToString(Errors.MAX_UINT_REACHED));
